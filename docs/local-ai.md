...
Three models exist: Llama 3.2 1B (default), Gemma 3 270M (lite, WASM fallback), and Llama 3.2 3B (opt-in large model). What a bundle carries depends on the platform (`bundlePlatforms` in the manifest, arch-qualified tags like `darwin-arm64`): Linux, Apple Silicon macOS, and Windows bundle GGUF builds and run them natively (llama.cpp's CPU backend replaces the lite fallback there; the Windows NSIS installer tops out near 2 GB, so Windows carries the 1B only). Intel macOS is the one desktop build still on the in-webview runtime: it bundles the ONNX Llamas plus the lite model. See "Native inference (desktop)" below.
...
- Models: GGUF entries in the manifest (`format: "gguf"`, `minDevice: "native"`; the 1B bundles on `["linux", "darwin-arm64", "win32"]`, the 3B skips win32 for the NSIS cap). One file per model; tokenizer and chat template ship inside it.
...
- Import: a pinned native model the installer excluded (the 3B on Windows) can be imported from a file. The AI Assistant settings offer "Import model file…"; the app verifies the file byte-for-byte against the manifest (`model_store.rs`), stores it under `app_data_dir()/models/`, and only then makes it loadable. Fully offline — this is the supported path for air-gapped environments. Bundle resources win when both copies exist; a "Remove" button reclaims the disk. Design: `docs/model-download-plan.md`.
...
- Desktop installers grow by the bundled weight size (roughly the manifest's `totalBytes` per included model). Current totals: ~2.8 GB of GGUF on Linux and Apple Silicon, ~0.8 GB of GGUF on Windows (the NSIS installer cap excludes the 3B, and the picker shows "(not available in this build)"), ~4.1 GB of ONNX on Intel macOS. Native bundles carry no webview models, so they have no lite fallback.
