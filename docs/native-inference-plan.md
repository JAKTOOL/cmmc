...
- Native bundles carry GGUF only. The ONNX entries (both Llamas and the Gemma lite) restrict their `bundlePlatforms` to the webview platforms (`darwin-x64`, `win32`) — on a native build, llama.cpp's CPU backend is the low-end fallback, so the WASM lite model has no role and its ~0.6 GB stays out of the installer. Net native bundle (Linux, Apple Silicon): ~2.8 GB GGUF. Trade-off: if the native engine itself fails on some machine, the app reports AI unavailable instead of degrading to the webview — accepted, because llama.cpp CPU init has essentially no failure modes that a webview on the same machine would survive.
...
5. Fallback: break the native path (rename the GGUF) and confirm the app reports the AI feature as unavailable cleanly — native bundles carry no webview models, so there is no lite fallback there by design.
